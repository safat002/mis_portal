...
(() => {
    ...
    window.initializeDataPrepWorkflow = async function(reportConfig) {
        ...
        try {
            const response = await fetch('/mis_app/data-prep-modal-content/');
            if (!response.ok) throw new Error('Could not load modal UI.');
            contentEl.innerHTML = await response.text();

            // Initialize the sub-modal and bind all event listeners
            fillMissingModal = new bootstrap.Modal(document.getElementById('fillMissingModal'));
            bindEventListeners();
    ...
    try {
        const response = await fetch('/mis_app/api/reports/profile_data/', {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-CSRFToken": csrfToken },
            body: JSON.stringify(DataPrepState.reportConfig),
        });
        ...
        DataPrepState.originalPreviewData = JSON.parse(JSON.stringify(result.preview_data));
        DataPrepState.currentPreviewData = JSON.parse(JSON.stringify(result.preview_data));
    ...
    function bindEventListeners() {
        ...
        contentEl.addEventListener('click', (e) => {
            if (e.target.id === 'addTransformationBtn') {
                openFillMissingModal();
            }
            if (e.target.id === 'addFillStepBtn') {
                addFillMissingStep();
            }
        });
        ...
        if (fillMethodSelect) {
            fillMethodSelect.addEventListener('change', () => {
                ...
                customValueContainer.style.display = fillMethodSelect.value === 'custom' ? 'block' : 'none';
            });
        }
    }
    ...
    function openFillMissingModal() {
        ...
        selectEl.innerHTML = columns.map(c => `<option value="${c}">${c}</option>`).join('');
        fillMissingModal.show();
    }

    function addFillMissingStep() {
    ...
    function applyFullRecipeToPreview() {
        ...
        let tempData = JSON.parse(JSON.stringify(DataPrepState.originalPreviewData));
        ...
        DataPrepState.recipe.forEach(step => {
            if (step.strategy === 'handle_missing') {
                ...
                const nonNullValues = tempData.map(row => row[col]).filter(v => v !== null && v !== undefined);
    ...
    function renderColumnList() {
        ...
        const columns = Object.keys(DataPrepState.columnMetadata);
        if (columns.length === 0) {
            columnListEl.innerHTML = '<div class="list-group-item text-muted">No columns found.</div>';
            return;
        }
        columnListEl.innerHTML = columns.map(col => {
            const meta = DataPrepState.columnMetadata[col];
            const hasIssues = meta.null_count > 0;
            const null_percent = meta.total_count > 0 ? (meta.null_count / meta.total_count * 100).toFixed(0) : 0;
            return `
                <a href="#" class="list-group-item list-group-item-action ${hasIssues ? 'list-group-item-warning' : ''}">
                    <div class="d-flex w-100 justify-content-between">
                        <h6 class="mb-1 fw-bold small">${col}</h6><small class="text-info">${meta.inferred_type}</small>
                    </div>
                    <div class="d-flex w-100 justify-content-between small">
                         <span class="text-muted">Missing: ${null_percent}%</span><span class="text-muted">Unique: ${meta.unique_count}</span>
                    </div>
                </a>`;
        }).join('');
    }
    ...
    function renderPreviewTable() {
        ...
        if (!data || data.length === 0) {
            previewContainer.innerHTML = '<p class="text-muted p-5 text-center">No data to display.</p>';
            return;
        }
        const headers = Object.keys(data[0]);
        let tableHtml = '<table class="table table-sm table-bordered table-hover"><thead><tr>';
        ...
        data.forEach((row, rowIndex) => {
            tableHtml += '<tr>';
            headers.forEach(h => {
                const originalValue = DataPrepState.originalPreviewData[rowIndex] ? DataPrepState.originalPreviewData[rowIndex][h] : undefined;
                const currentValue = row[h];
                const isChanged = String(originalValue) !== String(currentValue);
                const cellClass = isChanged ? 'table-success' : '';
                tableHtml += `<td class="${cellClass}">${currentValue === null ? '<em class="text-muted">null</em>' : currentValue}</td>`;
            });
            tableHtml += '</tr>';
        });
    ...
    function renderPipeline() {
        ...
        if (DataPrepState.recipe.length === 0) {
            pipelineListEl.innerHTML = '<p class="text-muted small text-center p-3">No cleaning steps have been applied yet.</p>';
            return;
        }

        pipelineListEl.innerHTML = DataPrepState.recipe.map((step, index) => {
