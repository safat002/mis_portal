...
class DataManagementApp {
    ...
    // Helper: safely get element by ID; optionally suppress warnings
    ...
    renderTableList() {
        ...
        const filteredTables = this.state.tables.filter(t => t.toLowerCase().includes(searchTerm));
    ...
    renderWelcomeView() {
        ...
        if (this.dom.workspace) {
            ...
            <div class="workspace-welcome">
                ...
                <p>Select a connection and table from the left to begin, or click the '+' button to add data.</p>
            </div>`;
        }
    }
...
sanitizeMessage(msg) {
                    ...
                    <tbody>
                        ${result.columns.map(col => `
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-outline-primary rename-col-btn" data-column="${col.name}">
                                        Rename
                                    </button>
                                    <button class="btn btn-sm btn-outline-danger delete-col-btn" data-column="${col.name}">
                                        Delete
                                    </button>
                                </td>
                            </tr>
    ...
    async handleAddColumn() {
        ...
        const columnName = columnNameEl.value;
        const columnType = columnTypeEl.value;
        ...
        try {
            await this.api.addColumn(this.state.currentConnectionId, this.state.selectedTable, columnName, columnType);
    ...
    handleDropTable() {
        this.setupPasswordConfirmation(
            ...
            async (password) => {
                await this.api.dropTable(this.state.currentConnectionId, this.state.selectedTable, password);
...
class ApiService {
    ...
    dropTable(connectionId, tableName, password) {
    ...
    addColumn(connectionId, tableName, columnName, columnType) {
        return this.fetch('/api/data/add-column/', {
            method: 'POST',
            body: { connection_id: connectionId, table_name: tableName, column_name: columnName, column_type: columnType }
        });
    }
